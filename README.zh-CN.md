...
**版本：** CrossMux 1.4.1（基于 CrossPoint Reader 1.4.1，并同步上游 `develop` 至 `63eda54e`）
...
- **Apps 应用中心**（首页的 `Apps` 菜单）：2048、扫雷、数独、五子棋、中国象棋，以及一个程序化生成的「Ugly Avatar」头像生成器。应用超过一屏时以页点分页。
- **微信读书**：扫码登录、浏览个人书架、下载图书，并以 EPUB 离线阅读。
