...
- **Sleep Screen Mode**: Which sleep screen to display when the device sleeps:
  ...
  - "Cover (Fit)" - The book cover image, scaled to fit centered on the screen
  - "Cover (Crop)" - The book cover image, scaled and cropped to fill the screen
  - "Cover + Custom" - The book cover image, falls back to "Custom" behavior if no book is open
  - "Blank" - A blank screen
- **Sleep Screen Filter**: What filter to apply to the sleep screen:
  - "None" (default) - Display the sleep screen as-is
  - "Contrast" - Display as black & white without grayscale (only applies to cover/custom image modes)
  - "Negative" - Invert the sleep screen colors
...
The **Sleep Screen Filter** applies a visual filter to the sleep screen:

- **None** (default): Display as-is. Cover and custom images are shown in grayscale.
- **Contrast**: Display as black & white without grayscale. Only applies to **Cover** and **Custom** modes.
- **Negative**: Invert all colors. Applies to all modes (e.g., dark background becomes light).
