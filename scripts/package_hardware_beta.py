...
def verify_firmware(path, board):
    with path.open('rb') as stream:
        ...
        if chip_id != ESP32S3_CHIP_ID:
            raise SystemExit(f'{path.name} chip ID is 0x{chip_id:04x}, expected ESP32-S3')

        marker = f'CROSSPOINT-BOARD-V1:{board};'.encode()
        overlap = b''
    ...
    raise SystemExit(f'{path.name} does not contain board tag {board!r}')
