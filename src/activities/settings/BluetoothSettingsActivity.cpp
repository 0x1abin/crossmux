...
void BluetoothSettingsActivity::handleMenuConfirm() {
  ...
  switch (action) {
    ...
    case Action::MapButtons:
      startActivityForResultWith<BleButtonMapActivity>([this](const ActivityResult&) {
        rebuildMenuRows();
        requestUpdate();
      });
      break;
...
void BluetoothSettingsActivity::render(RenderLock&&) {
  ...
  const char* confirm = view == View::Menu ? tr(STR_SELECT) : scanCanRestart ? tr(STR_BT_SCAN) : tr(STR_CONNECT);
