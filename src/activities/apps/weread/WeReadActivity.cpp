...
bool drawCachedCover(GfxRenderer& renderer, const std::string& bookDir, const Rect& bounds) {
  // Hold the storage lock for the whole exists+open+read+draw sequence. Each
  // HalStorage call takes the lock on its own, leaving unlock windows in which
  // the main task (core 0) can drive the same SD SPI bus mid-transaction. That
  // races SdFat's m_spiActive and trips FreeRTOS's xTaskPriorityDisinherit
  // assert (SMP cross-core give). Serializing the whole read prevents it.
...
void WeReadActivity::onEnter() {
  Activity::onEnter();
  LOG_DBG("WR", "onEnter: start");
  if (!WeReadBrowse::clearLegacyWorkspace()) LOG_ERR("WR", "legacy browse workspace cleanup failed");
  LOG_DBG("WR", "onEnter: before RenderLock");
  {
    RenderLock lock(*this);
    LOG_DBG("WR", "onEnter: RenderLock acquired");
    sdFontSystem.releaseLoadedFont(renderer);
    if (auto* fontCache = renderer.getFontCacheManager()) fontCache->clearCache();
    LOG_DBG("WR", "onEnter: fonts released");
  }
  LOG_DBG("WR", "onEnter: RenderLock released");
  ...
  LOG_DBG("WR", "onEnter: state reset done");
  if (!WeReadStore::hasAcceptedDisclaimer()) {
    state_.store(State::Disclaimer);
    LOG_DBG("WR", "onEnter: disclaimer state, requesting update");
    requestUpdate();
    return;
  }
  LOG_DBG("WR", "onEnter: setting pendingEnterApp_");
  pendingEnterApp_ = true;
  requestUpdate();
}
...
void WeReadActivity::handleDetailInput() {
  // Touch: the introduction text block above the action list selects the
  // Introduction action; the bottom action list rows select their action.
  const auto& metrics = UITheme::getInstance().getMetrics();
  ...
  if (mappedInput.wasScreenTouchDown(tx, ty)) {
    if (ty >= actions.y) {
      int row = -1;
      if (mappedInput.wasListItemTouchedDown(row, kDetailListActionCount, detailSelected_ - 1, actions.y,
                                             actions.height, false) &&
          row + 1 != detailSelected_) {
        detailSelected_ = row + 1;
        requestUpdate();
      }
      return;
    }
    if (ty >= content.y && detailSelected_ != static_cast<int>(DetailAction::Introduction)) {
  ...
  if (mappedInput.wasScreenTapped(tx, ty)) {
    if (ty >= actions.y) {
      int row = -1;
      if (mappedInput.wasListItemTapped(row, kDetailListActionCount, detailSelected_ - 1, actions.y, actions.height,
                                        false)) {
        detailSelected_ = row + 1;
...
bool WeReadActivity::handleTabBarTap() {
  int tx = 0;
  int ty = 0;
  if (!mappedInput.wasScreenTapped(tx, ty)) return false;
  const auto& metrics = UITheme::getInstance().getMetrics();
  const Rect safe = UITheme::getInstance().getScreenSafeArea(renderer, true, false);
  const Rect tabBar{safe.x, safe.y + metrics.topPadding + metrics.headerHeight, safe.width, metrics.tabBarHeight};
  int index = -1;
  if (GUI.tabIndexFromPoint(renderer, tabBar, mainTabs_, tx, ty, index)) {
    mainFocus_.store(MainFocus::Content);
    selectMainTab(index == 0 ? MainTab::Shelf : MainTab::Manage);
    return true;
  }
  // A tap below the tab bar drops focus into the content but must NOT consume
  // the tap: returning true here swallows taps on book covers / manage rows,
  // so handleShelfInput/handleManageInput never run and books never open.
  if (ty >= tabBar.y + tabBar.height) {
    mainFocus_.store(MainFocus::Content);
    requestUpdate();
    return false;
  }
...
void WeReadActivity::handleMainTabInput() {
  // Touch the tab bar: tapp a tab to focus and switch to it.
  if (handleTabBarTap()) return;
...
void WeReadActivity::handleManageInput() {
  // Touch the manage menu list.
  const auto& metrics = UITheme::getInstance().getMetrics();
  const Rect content = mainContentBounds();
  switch (handleListTouch(manageSelected_, kManageEntryCount, content.y, content.height, false)) {
    case ListTouchResult::Activated:
      activateManageEntry();
      return;
    case ListTouchResult::Consumed:
...
void WeReadActivity::activateManageEntry() {
...
void WeReadActivity::handleMainInput() {
  ...
  // Tab-bar taps must switch tabs regardless of the current focus, otherwise a
  // tap on "管理" while focus is on Content (the default) is swallowed by the
  // content handler below and the manage page never opens.
  if (handleTabBarTap()) return;
...
void WeReadActivity::handleShelfInput() {
  ...
  // Touch: touchdown previews the selection, a tap on a cover opens it.
  int tx = 0;
  int ty = 0;
  if (mappedInput.wasScreenTouchDown(tx, ty)) {
    const int touched = shelfIndexAt(tx, ty);
    if (touched >= 0) {
      if (shelfSelected_.load() != touched) moveShelfSelection(touched, itemsPerPage);
      return;
    }
  }
  if (mappedInput.wasScreenTapped(tx, ty)) {
    const int tapped = shelfIndexAt(tx, ty);
    if (tapped < 0) return;
    moveShelfSelection(tapped, itemsPerPage);
    resetShelfCoverLoading();
    activateSelected();
    return;
  }
...
int WeReadActivity::shelfIndexAt(const int x, const int y) const {
  const int count = static_cast<int>(std::min<uint32_t>(shelfCount_, INT32_MAX));
  if (count <= 0) return -1;
  const auto& metrics = UITheme::getInstance().getMetrics();
  const Rect content = mainContentBounds();
  ...
  for (int index = pageStart; index < pageEnd; ++index) {
    ...
    if (x >= coverX && x < coverX + layout.coverWidth && y >= coverY && y < coverY + layout.itemHeight) {
      return index;
    }
  }
  return -1;
...
void WeReadActivity::loop() {
  if (pendingEnterApp_) {
    LOG_DBG("WR", "loop: consuming pendingEnterApp_");
    pendingEnterApp_ = false;
    enterApp();
    // enterApp may have pushed a sub-activity (WifiSelection); if so, our loop
    // should not run this frame — ActivityManager will switch to it next.
    return;
  }
...
void WeReadActivity::render(RenderLock&&) {
  LOG_DBG("WR", "render: start, state=%d", static_cast<int>(state_.load()));
  downloadRenderPending_.store(false);
  stageRenderPending_.store(false);
