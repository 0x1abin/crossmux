...
- 使用微信扫码登录后可浏览或刷新书架。选择书籍后会缓存文字详情和 96×140 封面；文字先显示，封面随后加载。
- 详情页可选择是否缓存正文图片，再将整本书缓存为 EPUB。图片下载失败会跳过该图，章节或 SD 写入失败则中止整本缓存。
...
登录会话、书架、详情、章节、图片和缓存中间数据保存在 SD 卡的 `/.crosspoint/weread/`。详情和章节缓存可复用；旧章节缺少图片索引时会重新拉取。每本书的图片策略只在 EPUB 成功生成后保存。`session.bin` 含登录会话信息，请妥善保管 SD 卡；退出登录会清除本地会话和书架索引，已缓存的 EPUB 会保留。
