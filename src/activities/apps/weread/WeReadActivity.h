...
class WeReadActivity final : public Activity {
 ...
 private:
  ...
  // Deferred enterApp: onEnter() must not push a sub-activity (WifiSelection)
  // synchronously, because ActivityManager is still mid-push of WeRead itself
  // (pendingActivity would be overwritten and the WifiSelection dropped, which
  // left WeRead in a half-initialized state and crashed via the render mutex).
  // Set here, consumed by the first loop().
  bool pendingEnterApp_ = false;
  ...
  // Returns true if a screen tap landed on the tab bar and was consumed.
  bool handleTabBarTap();
  ...
  int shelfIndexAt(int x, int y) const;
  void activateManageEntry();
