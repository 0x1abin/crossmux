...
class InputManager {
 public:
  ...
  // Clear the one-shot tap/release edge events without delivering them. Used on
  // activity transitions so the new activity does not re-read a tap that the
  // previous activity already consumed within the same frame (InputManager clears
  // these in update(), but pushActivity runs mid-frame).
