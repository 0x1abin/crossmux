...
void InputManager::clearTouchTapEvent() {
  // Drop the one-shot tap/release edges without delivering them. Used on activity
  // transitions so the new activity does not re-read a tap the previous activity
  // already consumed within the same frame (InputManager clears these in update(),
  // but pushActivity runs mid-frame).
