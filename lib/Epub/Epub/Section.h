...
class Section {
 ...
 public:
  ...
  size_t residentBytes() const {
    ...
    if (build_) {
      total += sizeof(BuildContext) + build_->lut.capacity() * sizeof(PageLutEntry) + build_->parsePath.capacity() +
               build_->contentBase.capacity() + build_->imageBasePath.capacity() + build_->htmlPath.capacity() +
