...
- `section.bin`: **per-flavor** — Latin builds **Version 48**, Chinese builds (`ENABLE_CHINESE_VERSION`) **Version 49**. Versions 48/49 invalidate pagination after source whitespace began controlling CJK gaps, ruby boundaries began carrying inline continuation, and `<br>` stopped re-applying container margins. The same built-in font IDs resolve to different font data and metrics in the two firmware flavors, so independent counters prevent a reflash from reusing pagination generated by the other flavor.
