...
- `section.bin`: **per-flavor** — Latin builds **Version 54**, Chinese builds
  (`ENABLE_CHINESE_VERSION`) **Version 55**. Versions 54/55 invalidate cached
  word positions after soft-flush indentation became one-shot and the default
  CJK paragraph indent changed to two ideograph advances. The same built-in font
