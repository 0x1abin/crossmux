...
- `section.bin`: **per-flavor** — Latin builds **Version 52**, Chinese builds (`ENABLE_CHINESE_VERSION`) **Version 53**. Versions 52/53 reserve layout space for ruby/CJK justification and expand serialized footnote hrefs from 96 to 256 bytes. The same built-in font IDs resolve to different font data and metrics in the two firmware flavors, so independent counters prevent a reflash from reusing pagination generated by the other flavor.
